...
int ConfiguraSistema (TipoSistema *p_sistema) {
	...
	pinMode(23, OUTPUT);    // Se�al de audio
	softToneCreate(23);


	pinMode(IR_TX_PIN, OUTPUT);    // Se�al de activacion led disparo
	pinMode(IR_RX_PIN, INPUT);    // Se�al de activacion led disparo
	...
	while(1) {
		...
		if(kbhit()) {
			...
			switch(teclaPulsada) {
				...
				case 's':
					// Realiza la funcion InicializaDisparo
				...
				case 'i':
					// Realiza la funcion InicializaImpacto
					piLock(PLAYER_FLAGS_KEY);
...
int main ()
{
	...
	fsm_trans_t servo_basico[] = {
			{ WAIT_START, CompruebaComienzo, WAIT_KEY, ComienzaSistema },
			...
			{ WAIT_KEY, CompruebaTriggerButton, TRIGGER_BUTTON, DisparoIR },
			{ TRIGGER_BUTTON, CompruebaTimeoutDisparo, WAIT_KEY, FinalDisparoIR },
			{ TRIGGER_BUTTON, CompruebaImpacto, WAIT_KEY, ImpactoDetectado },
			{ WAIT_KEY, CompruebaFinalJuego, WAIT_END, FinalizaJuego },
			{ WAIT_END, CompruebaRestart, WAIT_START, ComienzaSistema },
			{-1, NULL, -1, NULL },
	};

	InicializaTeclado(&teclado);

	fsm_t* column_fsm = fsm_new (KEY_COL_1, columns, &teclado);
	fsm_t* keypad_fsm = fsm_new (KEY_WAITING, keypad, &teclado);
	fsm_t* player_fsm = fsm_new (WAIT_START, reproductor, &(sistema.player));
	fsm_t* servo_fsm = fsm_new (WAIT_START, servo_basico, &(sistema.torreta));
