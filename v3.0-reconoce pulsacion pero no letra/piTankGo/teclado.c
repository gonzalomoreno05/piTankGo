...
int CompruebaColumnTimeout (fsm_t* this) {
 ...
 void row_1_isr (void) {
	...
	/*if (millis () < debounceTime[TECLADO_FILA_1]) {
		debounceTime[TECLADO_FILA_1] = millis () + DEBOUNCE_TIME ;
		return;
	}
	*/
	while (digitalRead (TECLADO_FILA_1) == HIGH) {
	delay (1) ;
	}
	...
	teclado.teclaPulsada.row = TECLADO_FILA_1;
 ...
 void row_2_isr (void) {
	// Pin event (key / button event) debouncing procedure
	/*if (millis () < debounceTime[TECLADO_FILA_2]) {
		debounceTime[TECLADO_FILA_2] = millis () + DEBOUNCE_TIME ;
		return;
	}
*/
	 while (digitalRead (TECLADO_FILA_2) == HIGH) {
		 delay (1) ;
	 }
	piLock (FLAG_TECLADO);

	teclado.teclaPulsada.row = TECLADO_FILA_2;
	teclado.teclaPulsada.col = teclado.columna_actual;
 ...
 void row_3_isr (void) {
	// Pin event (key / button event) debouncing procedure
	/*if (millis () < debounceTime[TECLADO_FILA_3]) {
		debounceTime[TECLADO_FILA_3] = millis () + DEBOUNCE_TIME ;
		return;
	}
*/
	 while (digitalRead (TECLADO_FILA_3) == HIGH) {
	 	delay (1) ;
	 	}
	piLock (FLAG_TECLADO);

	teclado.teclaPulsada.row = TECLADO_FILA_3;
	teclado.teclaPulsada.col = teclado.columna_actual;
 ...
 void row_4_isr (void) {
	// Pin event (key / button event) debouncing procedure
	/*if (millis () < debounceTime[TECLADO_FILA_4]) {
		debounceTime[TECLADO_FILA_4] = millis () + DEBOUNCE_TIME ;
		return;
	}
*/
	 while (digitalRead (TECLADO_FILA_4) == HIGH) {
	 	delay (1) ;
	 	}
	piLock (FLAG_TECLADO);

	teclado.teclaPulsada.row = TECLADO_FILA_4;
	teclado.teclaPulsada.col = teclado.columna_actual;
 ...
 void process_key (fsm_t* this) {
	...
	switch(p_teclado->teclaPulsada.col){
		case COL_1:
			if (p_teclado->teclaPulsada.row == TECLADO_FILA_1 && p_teclado->teclaPulsada.col == COL_1){
				printf("\nKeypress 1!!!\n");
				fflush(stdout);
			...
			break;
		case COL_2:
			if (p_teclado->teclaPulsada.row == TECLADO_FILA_1){
				printf("\nKeypress 2!!!\n");
				fflush(stdout);
				p_teclado->teclaPulsada.row = -1;
				p_teclado->teclaPulsada.col = -1;
